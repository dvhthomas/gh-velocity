...
out=$(gh velocity version -r json 2>&1)
...
out=$(gh velocity flow lead-time 1 -R cli/cli --config "$CLI_CONFIG" -r json 2>&1)
echo "$out" | jq -e '.lead_time_seconds' >/dev/null 2>&1 && pass "flow lead-time json" || fail "flow lead-time json: $out"

out=$(gh velocity flow lead-time 1 -R cli/cli --config "$CLI_CONFIG" -r markdown 2>&1)
...
out=$(gh velocity quality release v2.65.0 -R cli/cli --config "$CLI_CONFIG" --since v2.64.0 --discover -r json 2>/dev/null)
...
out=$(gh velocity quality release v2.65.0 -R cli/cli --config "$CLI_CONFIG" --since v2.64.0 -r json 2>/dev/null)
...
out=$(gh velocity quality release v2.65.0 -R cli/cli --config "$CLI_CONFIG" --since v2.64.0 -r markdown 2>/dev/null)
...
out=$(gh velocity status my-week -R dvhthomas/gh-velocity --config "${REPO_ROOT}/docs/examples/dvhthomas-gh-velocity.yml" --since 30d -r json 2>&1)
...
out=$(gh velocity status my-week -R dvhthomas/gh-velocity --config "${REPO_ROOT}/docs/examples/dvhthomas-gh-velocity.yml" --since 30d -r markdown 2>&1)
