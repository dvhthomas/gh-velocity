...
Velocity becomes a simple count of completed items. All items are assessed.
...
Requires `project.url`. Items without a value (null) are not assessed. Negative values are treated as not assessed with a warning.
...
Requires `project.url`. Fetches both active and completed iterations, then matches items by their iteration field assignment.
...
Iteration windows are generated by stepping forward/backward from the anchor date in increments of `length`. Items are matched by their close/merge date falling within the window.
