...
How to measure effort per iteration. Three effort strategies: `count` (count items), `attribute` (labels like `size:M` with point values), or `numeric` (a project board number field). See [Setting Up Flow Velocity]({{< relref "/guides/velocity-setup" >}}) for a walkthrough.
...
- [Setting Up Flow Velocity]({{< relref "/guides/velocity-setup" >}}) -- configure effort-weighted sprint metrics
